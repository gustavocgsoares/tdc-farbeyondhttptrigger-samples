...
{
    public static class EcommerceOrderMixSendFinishEmailEventHubTrigger
    {
        [FunctionName("EcommerceOrderMixSendFinishEmailEventHubTrigger")]
        public static async Task Run(
            [EventHubTrigger("mix-send-finish-email", Connection = "EventHubConnectionAppSetting")] EventData[] events,
            ILogger log)
        {
            ...
            foreach (EventData eventData in events)
            {
                try
                {
                    ...
                    order.steps.sendFinishEmailDone = true;
        ...
        private static void SendEmail(dynamic order)
        {
            ...
            var subject = $"Pedido Mix {order.number} - NFe"; // assunto;
            var body = $"Olá {order.customer.name}. Sua nota fiscal foi gerada com o código {((JObject)order)["nfe"].ToString()} e seus produtos já estão à caminho. Grande abraço e te vejo no TDC 2020!"; // mensagem
